...
结合大语言模型 (LLM) 的智慧树（知到）全自动考试与作业助手。只需一次扫码，即可全自动获取课程列表、拉取考试任务，并调用 AI 完成高分作答与提交。告别繁琐的抓包，真正实现“零配置”开箱即用！
---
✨ 核心特性
🔒 扫码一键登录：内置获取二维码与轮询接口，跨平台自动调用系统自带看图软件打开二维码。直接使用微信或知到 APP 扫码，彻底免去小白用户抓取 `Cookie` 的痛苦。
🤖 AI 智能赋能：完美兼容 OpenAI 标准 API 格式。可自由接入各类大语言模型（如 GPT-4, DeepSeek, Kimi 等），单选、多选、判断题通杀，准确率极高。
🚀 全自动交互：自动拉取当前账号下所有课程，终端交互式选择目标课程，自动分析待考清单。
🛡️ 防风控设计：内置合理的请求间隔与随机休眠策略，模拟真实人类答题频率，保护账号安全。
...
---
🤝 参与贡献
欢迎大家提交 Pull Request 或 Issue！
智慧树的 API 可能会不定期更新迭代。如果你发现课程列表或考试列表获取失败，欢迎协助更新 `fetch_course_list` 和 `fetch_exam_list` 中的接口逻辑。
Fork 本仓库
